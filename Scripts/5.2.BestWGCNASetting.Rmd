...
SettingsSummary$setting <- paste0 ("Power", SettingsSummary$Power,
...
filenames <- list.files (recursive = TRUE, path = "Outputs/WGCNA_MergeModules", pattern = "All_genes_modules", full.names = FALSE)
grey_size <- c ()
for (file in 1:length (filenames)) {
  x <- read.csv (paste0 ("Outputs/WGCNA_MergeModules/", filenames [ file ]), header = F)
  grey_size [ gsub ("All_genes_modules_|.csv", "", filenames [ file ])  ] <- length (unique (x [ x$V2 == "grey", 1 ]))
}
